...
- (kxMovieError) openSubtitleStream: (NSInteger) subtitleStream
{
    ...
    if (codecDesc && (codecDesc->props & AV_CODEC_PROP_BITMAP_SUB)) {
        ...
        return kxMovieErrorUnsupported;
    }
...
- (void)thumbImageWithContentPath:(NSString *)path atTimestamp:(CGFloat)timestamp completeBlock:(void(^)(UIImage *thumbImage))block {
    NSAssert(path.length > 0, @"empty path");
    
    self.interruptCallback = ^BOOL(){
        return NO;
    };
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        if (!error && self.validVideo) {
            self.position = MIN(self.duration - 1, MAX(0, timestamp));
                                ...
                                if (_videoFrame->key_frame == 1) {
                                    ...
                                    if ([frame isKindOfClass:[KxVideoFrameRGB class]]) {
                                        ...
                                        if (provider) {
                                            CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
                                            if (colorSpace) {
                                                CGImageRef imageRef = CGImageCreate(rgbFrame.width,
                                                                                    rgbFrame.height,
                                                                                    8,
                                                                                    24,
                                                                                    rgbFrame.linesize,
